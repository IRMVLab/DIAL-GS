...
@torch.no_grad()
def perturb_positions(scene: Scene, render_func, render_args, env_map=None):
    """Translate Gaussians belonging to ID 0 to inspect motion robustness."""
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for viewpoint in tqdm(config["cameras"]):
                ...
                if id_mask_0.any():
                    scene.gaussians._xyz[id_mask_0, 1] += 0.03  # Shift upward slightly
                ...
                grid = make_grid(
                    [image_original, image_modified, image_dynamic, image_static],
                    nrow=4,
                    padding=0,
                )
...
if __name__ == "__main__":
    ...
    dynamic_id_dict = {}
    # Expected format: {"cam0": [id1, id2], "cam1": [...], ...}
    if args.dynmaic_id_dict_path and os.path.exists(args.dynmaic_id_dict_path):
        ...
        logging.info(
            f"Loaded dynamic_id_dict from {args.dynmaic_id_dict_path}, {len(dynamic_id_dict)} ids"
        )
    else:
    ...
    perturb_positions(scene, render_func, (args, background), env_map=env_map)
